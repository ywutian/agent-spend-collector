...
Optional AI triage: set `SPEND_TRIAGE_MODEL` (e.g. `gpt-4o-mini`) to append a likely
cause + recommended action to the alert, generated by an LLM from the alert metadata
only. It calls an OpenAI-compatible endpoint (`SPEND_TRIAGE_BASE_URL`, default OpenAI)
with `SPEND_TRIAGE_API_KEY` or `OPENAI_API_KEY`. Point the base URL at your own
gateway (`http://127.0.0.1:8788/openai/v1`) to route and record the triage call
itself; in that mode set `SPEND_TRIAGE_API_KEY` to the gateway token, or let it use
`SPEND_GATEWAY_TOKEN`. It never runs the detectors — those stay deterministic; this
only narrates a firing alert, is opt-in, and degrades to the plain alert on any
failure.
